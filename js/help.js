...
<div class="mku-tab-container" id="help-tab">
	...
	<div class="mku-tab-content" tab-title="編成表マスタ">
		<h2>マスタとは</h2>
		<p>本アプリで編成や車両を操作する場合、「編成解除」や「廃車」といった操作がありますが、これらの機能はあくまでも指定された年月で「編成解除」や「廃車」のタイムスタンプを押すだけのものです。アプリのメイン画面で見えなくなっても、年月を戻せば再表示されることからもわかるように、編成や車両の存在が消えるわけではありません。</p>
		<p>このように、本アプリのメイン画面で見えている編成表は、編成表の「データそのもの」ではありません。その大本となる「データそのもの」が別の場所で管理されており、そこには、これまでに製造された車両の製造や廃車の年月、改番や編成の組成の履歴など、全ての情報が記録されています。本アプリでは、この<b>「データそのもの」のことを「編成表マスタデータ」と呼んでいます</b>。本アプリのメイン画面で表示される編成表は、この「データそのもの」と指定された「現在年月」をもとに、表示すべき情報の取捨選択を行って生成されたものです。</p>
		<h2>マスタ管理</h2>
		<p>「マスタデータ」を直接編集することができるのが「マスタ管理」機能です。</p>
		<p>マスタ管理ダイアログでは、解除された編成や廃車された車両も含め、現在開いている編成表データに存在する全てのデータを一覧で確認し、編集･削除することができます。</p>
		<p>「マスタ管理」機能では、「編成解除」や「廃車」といった通常発生しうる動作ではなく、最初からなかったことにする、いわば「歴史修正」を行うことができます。また、「廃車」や「編成解除」などの取り消しや、年月の変更なども行うことができます。</p>
		<h2>マスタ編集を行う上での注意事項</h2>
		<p>この機能で実施できる操作は非常に強力なものです。通常の操作と異なり、データの整合性まで保証するものではありません。操作方法によっては、データが破損したり、最悪の場合は編成表マネージャが正常に動作しなくなることもあります。また、この機能で削除･編集した編成･車両は復元できなくなります。編成表マスタへの操作はくれぐれも慎重に行ってください。</p>
		<h2>保存</h2>
