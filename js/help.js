...
<div class="mku-tab-container" id="help-tab">
	...
	<div class="mku-tab-content" tab-title="編成表データ">
		<h2>保存</h2>
		<p>本アプリで作成した編成表データを保存したい場合、「保存」ボタンを押すことでJSON形式での保存が可能です。本アプリで編集中のデータはブラウザの更新ボタンを押下すると消えてしまうため、定期的にJSONとして保存しておくことをおすすめします。</p>
		<h2>読込</h2>
		<p>「読込」ボタンから、PC上に存在するJSONファイルから編成表データの読み込みを行うことができます。編集中のデータが存在する状態でこの操作を実行すると、編集中のデータはすべて消えてしまいますので、必ず保存を行ってから読み込みを行うようにしてください。</p>
		<h2>マスタ管理</h2>
		<p>本アプリのメイン画面で見えている編成表は、編成表のデータそのものではなく、そのデータと指定されている現在年月をもとに編集されたテーブルです。編集された編成表ではなく、その大本となる「データそのもの」のことは「マスタデータ」と呼んでいます。</p>
		<p>本アプリで編成や車両を操作する場合、できる操作は「編成解除」や「廃車」です。これはあくまでも、現在年月で「編成解除」や「廃車」のタイムスタンプを押すだけで、車両の存在が消えるわけではありません。</p>
		<p>作ってしまった編成や車両の存在が最初からなかったことにしたい、完全削除したい場合に使用するのが「マスタ管理」機能です。マスタ管理ダイアログでは、過去に存在した編成や車両も含め、現在編集している編成表データに存在する全てのデータを一覧で確認し、編集･削除することができます。</p>
